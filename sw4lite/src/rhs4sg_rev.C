...
void rhs4sg_rev_onesided5( int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
		 int nk, int* __restrict__ onesided, float_sw4* __restrict__ a_acof, float_sw4 *__restrict__ a_bope,
		 float_sw4* __restrict__ a_ghcof, float_sw4* __restrict__ a_lu, float_sw4* __restrict__ a_u,
		 float_sw4* __restrict__ a_mu, float_sw4* __restrict__ a_lambda, 
		 float_sw4 h, float_sw4* __restrict__ a_strx, float_sw4* __restrict__ a_stry, 
...
void rhs4sg_rev(int mNumberOfCartesianGrids, vector<int> iFirst, vector<int> iLast, vector<int> jFirst, vector<int> jLast, vector<int> kFirst, vector<int> kLast, vector<int> nk, vector<int*> onesided, float_sw4* a_acof, float_sw4* a_bope, float_sw4* a_ghcof, vector<Sarray>& a_Lu, vector<Sarray>& a_U, vector<Sarray>& a_Mu, vector<Sarray>& a_Lambda, vector<double> mGridSize, vector<double*> m_sg_str_x, vector<double*> m_sg_str_y, vector<double*> m_sg_str_z  )
{
...
//#pragma omp parallel 
{
   float_sw4 mux1, mux2, mux3, mux4, muy1, muy2, muy3, muy4, muz1, muz2, muz3, muz4;
    ...
    {
        float_sw4* __restrict__ a_mu = a_Mu[g].c_ptr();
        float_sw4* __restrict__ a_lambda = a_Lambda[g].c_ptr();
        float_sw4* __restrict__ a_lu = a_Lu[g].c_ptr();
        float_sw4* __restrict__ a_u = a_U[g].c_ptr();
        float_sw4* __restrict__ a_strx = m_sg_str_x[g];
        float_sw4* __restrict__ a_stry = m_sg_str_y[g];
        float_sw4* __restrict__ a_strz = m_sg_str_z[g];

        float_sw4 h = mGridSize[g];
        const int cof = 1.0/(h*h);
        const int ifirst = iFirst[g];
        const int ilast = iLast[g];
        const int jfirst = jFirst[g];
        const int jlast = jLast[g];
        const int kfirst = kFirst[g];
        const int klast = kLast[g];
...
#pragma ivdep
        ...
        rhs4sg_rev_onesided5(ifirst,ilast,jfirst,jlast,kfirst,klast,nk[g], onesided[g], a_acof, a_bope, a_ghcof, a_lu, a_u, a_mu, a_lambda, h, a_strx, a_stry, a_strz );
        rhs4sg_rev_onesided4(ifirst,ilast,jfirst,jlast,kfirst,klast,nk[g], onesided[g], a_acof, a_bope, a_ghcof, a_lu, a_u, a_mu, a_lambda, h, a_strx, a_stry, a_strz );
    }
} // OMP Parallel
